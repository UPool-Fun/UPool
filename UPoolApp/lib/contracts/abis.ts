// UPool Contract ABIs - Frontend Only
// Minimal ABIs copied from deployed contracts in UPoolContracs

import { Address } from 'viem'

// ✅ **CONTRACT ADDRESSES** - Base Mainnet (Deployed January 26, 2025)
...
// ✅ **POOL ABI** - Individual pool interactions
...
// ✅ **TYPES** - Frontend enums and interfaces
...
// Frontend interfaces (simplified for UI use)
export interface PoolData {
  title: string
  description: string
  fundingGoal: string  // ETH string for frontend forms
...
export interface Milestone {
  ...
  title: string
  description: string
  percentage: number  // 0-100 for frontend display
  amount: string     // ETH string for frontend display
}
...
// ✅ **HELPER FUNCTIONS** - Minimal utilities for frontend
export const formatEthAmount = (amount: string): string => {
  return parseFloat(amount).toFixed(4) + ' ETH'
}

export const getRiskStrategyLabel = (strategy: RiskStrategy): string => {
  switch (strategy) {
...
export const getApprovalMethodLabel = (method: ApprovalMethod): string => {
  switch (method) {
